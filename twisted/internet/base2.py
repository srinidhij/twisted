...
class ReactorBase(object):
    ...
    def wakeUp(self):
        ...
        mdata = {'event':'wakeUp', 'msg':'Wakingup the event loop', 'level':'info'}
    ...
    def stop(self):
        ...
        #monitor
        mdata = {'event' : 'stop',
           'msg' : 'Stopping the reactor event loop',
           'level' : 'critical'
        }
        ...
        #monitor
        mdata = {'event' : 'stop',
           'msg' : 'Reactor event loop stopped',
           'level' : 'critical'
        }
    ...
    def crash(self):
        ...
        mdata = {
            'event' : 'crash',
            'msg' : '''Reset reactor state tracking attributes and re-initialize certain
        state-transition helpers which were set up in __init__ but later
        destroyed''',
            'level' : 'critical'
        }
    ...
    def sigBreak(self, *args):
        ...
        mdata = {
            'event':'sigBreak' ,
            'msg' : ' Handling SIGBREAK interrupt.' ,
            'level' : 'warning'
        }
    ...
    def sigTerm(self, *args):
        ...
        mdata = {
            'event':'sigTerm' ,
            'msg' : ' Handling SIGTERM interrupt.' ,
            'level' : 'critical'
        }
    ...
    def fireSystemEvent(self, eventType):
        ...
        mdata = {
            'event':'fireSystemEvent' ,
            'msg' : 'Firing event %s'%event ,
            'level' : 'info'
        }

        sender.send(str(mdata) , MONITOR_ADDR)     
        if event is not None:
            log.msg('firing event from reactor'+str(event))
            event.fireEvent()
